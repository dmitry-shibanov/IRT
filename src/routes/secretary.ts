...
import { getStudentById, getStudents, postCreateUser, getInitialSuitableTable, postSubjectsToSearch } from '../controllers/secretary';
...
router.get('/students', sessionSecretary.bind(null, keys.secretary), getStudents);
...
router.get('/findsuitablestudent', sessionSecretary.bind(null, keys.secretary), getInitialSuitableTable);

// post
router.post('/createuser', sessionSecretary.bind(null, keys.secretary), [check('email').isEmail(), body('password').isAlphanumeric().isLength({min: 7})], postCreateUser);
router.post('/searchresult', sessionSecretary.bind(null, keys.secretary), [check('factors').isArray()], postSubjectsToSearch);
